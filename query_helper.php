<?php
    ...
    switch ($_POST["query"]) 
    {
        ...
        case 'insert_client':
            run_query("insert into account values(".$_POST["acno"].", '".$_POST["pass"]."', '".$_POST["name"]."', ".$_POST["phone"].", '".$_POST["addr"]."', ".$_POST["age"].")", true);
            run_query("insert into balance values(".$_POST["acno"].", ".$_POST["bal"].")", true);
        break;
        
        case 'get_bal' :
            run_query("select AccBal as balance from balance where AccNo=".$_POST["acno"],false);
        break;
        ...
        case 'check_ben':
            run_query("select count(*) as beneficiaries from beneficiary where AccSource=".$_POST["source"]." and AccDest=".$_POST["dest"],false);
        break;

        case 'add_ben':
            run_query("insert into beneficiary values(".$_POST["source"].", ".$_POST["dest"].")", true);
        break;

        case 'del_ben':
            run_query("delete from beneficiary where AccSource=".$_POST["source"]." and AccDest=".$_POST["dest"], true);
        break;
        
        case 'get_ben':
            run_query("select AccDest as ben from beneficiary where AccSource=".$_POST["acno"],false);
        break;

        case 'transact':
            run_query("insert into transactions values('".$_POST["dat"]."', ".$_POST["source"].", ".$_POST["dest"].", ".$_POST["amount"].")", true);
            run_query("update balance set AccBal = AccBal - ".$_POST["amount"]." where AccNo=".$_POST["source"], true);
            run_query("update balance set AccBal = AccBal + ".$_POST["amount"]." where AccNo=".$_POST["dest"], true);
        break;
    }
?>
